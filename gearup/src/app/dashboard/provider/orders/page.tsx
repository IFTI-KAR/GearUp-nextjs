    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-white">Manage Incoming Rental Orders</h1>
          <p className="text-xs text-slate-400">Confirm new requests, mark items as picked up or returned</p>
        </div>
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
        {isLoading ? (
          <div className="h-64 rounded-2xl bg-slate-900 animate-pulse border border-slate-800" />
        ) : orders.length === 0 ? (
          <div className="text-center py-12 space-y-2">
            ...
            <p className="text-xs text-slate-400">When customers place rental bookings for your gear, they will appear here.</p>
          </div>
        ) : (
                    ...
                    <td className="p-3 text-right whitespace-nowrap">
                      {order.status === 'PLACED' && (
                        <button
                          ...
                          className="px-3.5 py-1.5 rounded-xl bg-cyan-500 text-slate-950 font-extrabold text-xs hover:bg-cyan-400 transition shadow-md shadow-cyan-500/20 inline-flex items-center gap-1"
                        >
                      ...
                      {order.status === 'PAID' && (
                        <button
                          ...
                          disabled={updateStatusMutation.isPending}
                          className="px-3.5 py-1.5 rounded-xl bg-emerald-500 text-slate-950 font-extrabold text-xs hover:bg-emerald-400 transition shadow-md shadow-emerald-500/20 inline-flex items-center gap-1"
                        >
                      ...
                      {order.status === 'PICKED_UP' && (
                        <button
                          ...
                          disabled={updateStatusMutation.isPending}
                          className="px-3.5 py-1.5 rounded-xl bg-slate-700 text-white font-extrabold text-xs hover:bg-slate-600 transition inline-flex items-center gap-1"
                        >
                      ...
                      {order.status === 'RETURNED' && (
                        <span className="text-[11px] text-slate-400 font-medium flex items-center justify-end gap-1">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Rental Fulfilled
                        </span>
                      )}
                    </td>
                  </tr>
