...
import { Search, Filter, SlidersHorizontal, RefreshCw, Compass } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Browse & Rent Sports Gear</h1>
          <p className="text-xs sm:text-sm text-slate-400 mt-1">Filter by category, daily rate, brand, or location</p>
        </div>
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-900/90 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500 transition"
          />
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 p-4 rounded-2xl bg-slate-900/60 border border-slate-800">
        ...
        <div className="flex flex-wrap items-center gap-4 text-xs">
          ...
          <div className="flex items-center gap-2 bg-slate-950/60 px-3 py-1.5 rounded-xl border border-slate-800">
            ...
            <span className="text-slate-400 font-medium">Max Daily Rate:</span>
            <span className="font-extrabold text-emerald-400">${maxPrice}</span>
            <input
              type="range"
              min="10"
              max="200"
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-950/80 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-white focus:outline-none focus:border-emerald-500 cursor-pointer"
            >
          ...
          <button
            ...
            className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-slate-400 hover:text-white hover:border-slate-700 transition"
            title="Reset Filters"
          >
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 glass-panel rounded-3xl border border-slate-800 space-y-4">
          ...
          <h3 className="text-lg font-bold text-white">No gear items found</h3>
